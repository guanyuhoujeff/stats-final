...
這是一個針對商管背景學生的 Python 程式設計入門教材，旨在引導初學者使用 Python 進行基本的股票數據分析與投資組合管理。

本專案的核心檔案是 `investment_analysis_tutorial.ipynb`，它從 Python 基礎語法開始，逐步介紹 Pandas 資料處理、資料視覺化，最終實現一個包含三支股票的投資組合的預期報酬率與風險計算。
...
- `tw-price.txt`：包含三支台股（中鋼、台積電、國泰金）歷史股價的資料檔案。
...
4. **上傳資料檔**：筆記本成功開啟後，點擊左側的「檔案」圖示打開檔案總管。點擊「上傳至工作階段儲存空間」圖示，並選擇 `tw-price.txt` 檔案上傳。
   > **注意：** 請確保 `tw-price.txt` 與筆記本在同一個目錄層級，否則筆記本中的讀取路徑會找不到檔案。
